...
class ModelTestCase(IsolatedAsyncioTestCase):
    ...
    async def test_create_model(self):
            ...
            with patch("aiopynamodb.connection.TableConnection.describe_table") as req:
                ...
                with self.assertRaises(TableError):
                    await UserModel.create_table(
                        read_capacity_units=2, write_capacity_units=2, wait=True
                    )
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = MODEL_TABLE_DATA
            await RegionSpecificModel.create_table(
                read_capacity_units=2, write_capacity_units=2
            )
            self.assertEqual(
                RegionSpecificModel._connection.connection.region, "us-west-1"
            )
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = MODEL_TABLE_DATA
            await HostSpecificModel.create_table(
                read_capacity_units=2, write_capacity_units=2
            )
            self.assertEqual(
                HostSpecificModel._connection.connection.host, "http://localhost"
            )
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = MODEL_TABLE_DATA
            await BillingModeOnDemandModel.create_table(
                read_capacity_units=2, write_capacity_units=2
            )
        ...
        with patch(PATCH_METHOD, new=mock_wait) as req:
            ...
            self.assertEqual(
                actual["ProvisionedThroughput"], params["ProvisionedThroughput"]
            )
            ...
            self.assert_dict_lists_equal(
                sorted(
                    actual["AttributeDefinitions"], key=lambda x: x["AttributeName"]
                ),
                sorted(
                    params["AttributeDefinitions"], key=lambda x: x["AttributeName"]
                ),
            )
    ...
    async def test_model_attrs(self):
        ...
        self.assertEqual(
            repr(item),
            "UserModel(callable_field=42, email='needs_email', custom_user_name='foo', user_id='bar')",
        )
    ...
    async def test_save(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = {}
            await item.save(
                UserModel.email.does_not_exist() & UserModel.zip_code.exists()
            )
            args = req.call_args[0][1]
            params = {
                "Item": {
                ...
                "ExpressionAttributeNames": {"#0": "email", "#1": "zip_code"},
                "ReturnConsumedCapacity": "TOTAL",
                "TableName": "UserModel",
            }
            deep_eq(args, params, _assert=True)
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = {}
            await item.save(
                (UserModel.custom_user_name == "bar")
                | UserModel.zip_code.does_not_exist()
            ...
            params = {
                ...
                "ExpressionAttributeValues": {":0": {"S": "bar"}, ":1": {"S": "@"}},
                "ReturnConsumedCapacity": "TOTAL",
                "TableName": "UserModel",
            }
            deep_eq(args, params, _assert=True)
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            params = {
                ...
                "ExpressionAttributeNames": {"#0": "user_name"},
    ...
    async def test_index_count(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            params = {
                ...
                "ExpressionAttributeNames": {"#0": "user_id", "#1": "user_name"},
    ...
    async def test_query_limit_less_than_available_items_multiple_page(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            req.side_effect = [
                {
                    ...
                    "ScannedCount": 20,
                    "Items": items[:10],
                    "LastEvaluatedKey": {"user_id": "x"},
                },
                {
                    "Count": 10,
                    "ScannedCount": 20,
                    "Items": items[10:20],
                    "LastEvaluatedKey": {"user_id": "y"},
                },
                {
                    "Count": 10,
                    "ScannedCount": 20,
                    "Items": items[20:30],
                    "LastEvaluatedKey": {"user_id": "z"},
                },
            ]
            results_iter = UserModel.query("foo", limit=25)
            ...
            self.assertEqual(
                results_iter.last_evaluated_key, {"user_id": items[24]["user_id"]}
            )
    ...
    async def test_query_with_exclusive_start_key(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            self.assertEqual(
                results_iter.last_evaluated_key, {"user_id": items[9]["user_id"]}
            )
            ...
            self.assertEqual(req.mock_calls[0][1][1]["Limit"], 10)
            self.assertEqual(
                results_iter.last_evaluated_key, {"user_id": items[19]["user_id"]}
            )
    ...
    async def test_query(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            async for item in UserModel.query(
                "foo", UserModel.user_id.between("id-1", "id-3")
            ):
                hash_key, range_key = item._get_serialized_keys()
                queried.append(range_key)
            self.assertListEqual(
                [item.get("user_id").get(STRING) for item in items], queried
            )
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            queried = []
            async for item in UserModel.query(
                "foo", UserModel.user_id.startswith("id")
            ):
                queried.append(item.serialize())
            self.assertTrue(len(queried) == len(items))
        ...
        def fake_query(*args):
            ...
            if start_key:
                item_idx = 0
                for query_item in BATCH_GET_ITEMS.get(RESPONSES).get(
                    UserModel.Meta.table_name
                ):
                    item_idx += 1
                    if query_item == start_key:
                        break
                query_items = BATCH_GET_ITEMS.get(RESPONSES).get(
                    UserModel.Meta.table_name
                )[item_idx : item_idx + 1]
            else:
                query_items = BATCH_GET_ITEMS.get(RESPONSES).get(
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            async for item in UserModel.query(
                ...
                UserModel.email.contains("@")
                & UserModel.picture.exists()
                & UserModel.zip_code.between(2, 3),
            ):
                queried.append(item.serialize())
            params = {
    ...
    async def test_query_with_discriminator(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            params = {
                ...
                "ExpressionAttributeNames": {"#0": "id", "#1": "cls"},
                "ExpressionAttributeValues": {
                    ":0": {"S": "foo"},
                    ":1": {"S": "Child"},
                    ":2": {"S": "Grandchild"},
    ...
    async def test_scan(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            self.assertListEqual(
                [item.get("user_id").get(STRING) for item in items], scanned_items
            )
        ...
        def fake_scan(*args):
            ...
            if start_key:
                item_idx = 0
                for scan_item in BATCH_GET_ITEMS.get(RESPONSES).get(
                    UserModel.Meta.table_name
                ):
                    item_idx += 1
                    if scan_item == start_key:
                        break
                scan_items = BATCH_GET_ITEMS.get(RESPONSES).get(
                    UserModel.Meta.table_name
                )[item_idx : item_idx + 1]
            else:
                scan_items = BATCH_GET_ITEMS.get(RESPONSES).get(
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            async for item in UserModel.scan(attributes_to_get=["email"]):
                self.assertIsNotNone(item)
            params = {
                "ReturnConsumedCapacity": "TOTAL",
    ...
    async def test_get(self):
        ...
        def fake_dynamodb(*args):
            ...
            if kwargs == {"TableName": UserModel.Meta.table_name}:
                ...
                "ReturnConsumedCapacity": "TOTAL",
                "TableName": "UserModel",
                "Key": {"user_name": {"S": "foo"}, "user_id": {"S": "bar"}},
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            item = await UserModel.get("foo", "bar")
            self.assertEqual(item._get_keys(), {"user_id": "bar", "user_name": "foo"})
            params = {
                "ConsistentRead": False,
                "Key": {"user_id": {"S": "bar"}, "user_name": {"S": "foo"}},
                "ReturnConsumedCapacity": "TOTAL",
                "TableName": "UserModel",
            }
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            try:
                ...
                self.fail(
                    "UserModel.Exception must derive from aiopynamodb.Exceptions.DoesNotExist"
                )

        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = {
                "ConsumedCapacity": {"CapacityUnits": 0.5, "TableName": "UserModel"}
            }
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            self.assertEqual(
                item.overidden_attr, CUSTOM_ATTR_NAME_ITEM_DATA["Item"]["foo_attr"]["S"]
            )
            self.assertEqual(
                item.overidden_user_name,
                CUSTOM_ATTR_NAME_ITEM_DATA["Item"]["user_name"]["S"],
            )
            self.assertEqual(
                item.overidden_user_id,
                CUSTOM_ATTR_NAME_ITEM_DATA["Item"]["user_id"]["S"],
            )
    ...
    async def test_batch_get(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            async for item in SimpleUserModel.batch_get(item_keys):
                self.assertIsNotNone(item)
            req.call_args[0][1]["RequestItems"]["SimpleModel"]["Keys"].sort(
                key=json.dumps
            )
            params = {
                "ReturnConsumedCapacity": "TOTAL",
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            req.return_value = SIMPLE_BATCH_GET_ITEMS
            item_keys = ["hash-{}".format(x) for x in range(10)]
            async for item in SimpleUserModel.batch_get(
                item_keys, attributes_to_get=["numbers"]
            ):
                self.assertIsNotNone(item)
            req.call_args[0][1]["RequestItems"]["SimpleModel"]["Keys"].sort(
                key=json.dumps
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            item_keys = ["hash-{}".format(x) for x in range(10)]
            async for item in SimpleUserModel.batch_get(
                item_keys, consistent_read=True
            ):
                self.assertIsNotNone(item)
            req.call_args[0][1]["RequestItems"]["SimpleModel"]["Keys"].sort(
                key=json.dumps
        ...
        def fake_batch_get(*batch_args):
            ...
            if REQUEST_ITEMS in kwargs:
                batch_item = (
                    kwargs.get(REQUEST_ITEMS)
                    .get(UserModel.Meta.table_name)
                    .get(KEYS)[0]
                )
                batch_items = (
                    kwargs.get(REQUEST_ITEMS)
                ...
                response = {
                    UNPROCESSED_KEYS: {UserModel.Meta.table_name: {KEYS: batch_items}},
                    RESPONSES: {UserModel.Meta.table_name: [batch_item]},
                }
                return response
            return {}
    ...
    async def test_batch_write(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            with self.assertRaises(ValueError):
                async with UserModel.batch_write(auto_commit=False) as batch:
                    items = [
                        UserModel("hash-{}".format(x), "{}".format(x))
                        for x in range(26)
                    ]
                    for item in items:
            ...
            async with UserModel.batch_write(auto_commit=False) as batch:
                items = [
                    UserModel("hash-{}".format(x), "{}".format(x)) for x in range(25)
                ]
                for item in items:
                    await batch.delete(item)
                with pytest.raises(ValueError):
            ...
            async with UserModel.batch_write() as batch:
                items = [
                    UserModel("hash-{}".format(x), "{}".format(x)) for x in range(30)
                ]
                for item in items:
                    await batch.delete(item)
    ...
    async def test_batch_write_raises_put_error(self):
        items = []
        for idx in range(10):
            items.append(BatchModel("{}".format(idx)))
    ...
    async def test_index_queries(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            async for item in IndexedModel.email_index.query(
                "foo",
            ...
            params = {
                "KeyConditionExpression": "#0 = :0",
                "FilterExpression": "begins_with (#1, :1)",
                "ExpressionAttributeNames": {"#0": "email", "#1": "user_name"},
                "ExpressionAttributeValues": {":0": {"S": "foo"}, ":1": {"S": "bar"}},
    ...
    async def test_multiple_indices_share_non_key_attribute(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            await IndexedModel.create_table(
                read_capacity_units=2, write_capacity_units=2
            )
            params = {
                "AttributeDefinitions": [
    ...
    async def test_local_index(self):
        ...
        self.assertEqual(
            schema["local_secondary_indexes"][0]["projection"]["ProjectionType"],
            "INCLUDE",
        )
        self.assertEqual(
            schema["local_secondary_indexes"][0]["projection"]["NonKeyAttributes"],
            ["numbers"],
        )
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            await LocalIndexedModel.create_table(
                read_capacity_units=2, write_capacity_units=2
            )
            ...
            self.assertTrue(
                "ProvisionedThroughput" not in args["LocalSecondaryIndexes"][0]
            )
    ...
    def _get_office_employee(self):
        justin = Person(fname="Justin", lname="Phillips", age=31, is_male=True)
        loc = Location(lat=37.77461, lng=-122.3957216, name="Lyft HQ")
    ...
    def _get_grocery_list(self):
        return GroceryList(
            store_name="Haight Street Market",
    ...
    def _get_office(self):
        justin = Person(fname="Justin", lname="Phillips", age=31, is_male=True)
        lei = Person(fname="Lei", lname="Ding", age=32, is_male=True)
        garrett = Person(fname="Garrett", lname="Heel", age=30, is_male=True)
        tanya = Person(fname="Tanya", lname="Ashkenazi", age=30, is_male=False)
        loc = Location(lat=37.77461, lng=-122.3957216, name="Lyft HQ")
        emp1 = OfficeEmployeeMap(
            office_employee_id=123, person=justin, office_location=loc
        )
        emp2 = OfficeEmployeeMap(
            office_employee_id=124, person=lei, office_location=loc
        )
        emp3 = OfficeEmployeeMap(
            office_employee_id=125, person=garrett, office_location=loc
        )
        emp4 = OfficeEmployeeMap(
            office_employee_id=126, person=tanya, office_location=loc
        )
        return Office(office_id=3, address=loc, employees=[emp1, emp2, emp3, emp4])
    ...
    async def test_model_with_invalid_data_does_not_validate__list_attr(self):
        office = Office(
            office_id=3,
            address=Location(lat=37.77461, lng=-122.3957216, name="Lyft HQ"),
        )
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock):
            ...
            assert (
                str(cm.exception)
                == "Attribute 'employees.[0].person.fname' cannot be None"
            )
    ...
    async def test_model_with_maps_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.person.fname,
                GET_OFFICE_EMPLOYEE_ITEM_DATA.get(ITEM)
                .get("person")
                .get(MAP)
                .get("firstName")
                .get(STRING),
    ...
    async def test_model_with_maps_with_nulls_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.person.fname,
                GET_OFFICE_EMPLOYEE_ITEM_DATA_WITH_NULL.get(ITEM)
                .get("person")
                .get(MAP)
                .get("firstName")
                .get(STRING),
    ...
    async def test_model_with_list_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.store_name,
                GET_GROCERY_LIST_ITEM_DATA.get(ITEM).get("store_name").get(STRING),
            )
            self.assertEqual(
                item.groceries[2],
                GET_GROCERY_LIST_ITEM_DATA.get(ITEM)
                .get("groceries")
                .get(LIST)[2]
                .get(STRING),
            )
    ...
    async def test_complex_model_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.key, int(COMPLEX_MODEL_ITEM_DATA.get(ITEM).get("key").get(NUMBER))
            )
            self.assertEqual(item.key, 123)
            self.assertEqual(
                item.person.fname,
                COMPLEX_MODEL_ITEM_DATA.get(ITEM)
                .get("weird_person")
                .get(MAP)
                .get("firstName")
                .get(STRING),
    ...
    async def test_car_model_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.car_id,
                int(FULL_CAR_MODEL_ITEM_DATA.get(ITEM).get("car_id").get(NUMBER)),
            )
    ...
    async def test_car_model_with_null_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.car_id,
                int(CAR_MODEL_WITH_NULL_ITEM_DATA.get(ITEM).get("car_id").get(NUMBER)),
            )
    ...
    async def test_invalid_car_model_with_null_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(
                item.car_id,
                int(
                    INVALID_CAR_MODEL_WITH_NULL_ITEM_DATA.get(ITEM)
                    .get("car_id")
                    .get(NUMBER)
                ),
            )
    ...
    def test_serializing_map_with_null_check(self):
        ...
        # now with null check
        with pytest.raises(
            Exception, match="Attribute 'left.left.left.value' cannot be None"
        ):
            item.serialize(null_check=True)
        ...
        # now with null check
        with pytest.raises(
            Exception, match=r"Attribute 'leaves.\[0\].value' cannot be None"
        ):
            item.serialize(null_check=True)
    ...
    def test_raw_map_serialize_fun_one(self):
        map_native = {
            "foo": "bar",
            "num": 12345678909876543211234234324234,
            "bool_type": True,
            "other_b_type": False,
            "floaty": 1.2,
            "listy": [1, 2, 3],
        ...
        expected = {
            "M": {
                "foo": {"S": "bar"},
                "listy": {"L": [{"N": "1"}, {"N": "2"}, {"N": "3"}]},
                "num": {"N": "12345678909876543211234234324234"},
    ...
    def test_raw_map_deserializes(self):
        map_native = {
            ...
            "other_b_type": False,
            "floaty": 1.2,
            "listy": [1, 2, 12345678909876543211234234324234],
            "mapy": {"baz": "bongo"},
        }
        map_serialized = {
            "M": {
                ...
                "listy": {
                    "L": [
                        {"N": "1"},
                        {"N": "2"},
                        {"N": "12345678909876543211234234324234"},
                    ]
                },
                "mapy": {"M": {"baz": {"S": "bongo"}}},
            }
    ...
    async def test_raw_map_as_sub_map_from_raw_data_works(self):
        ...
        fake_db = self.database_mocker(
            ExplicitRawMapAsMemberOfSubClass,
            EXPLICIT_RAW_MAP_MODEL_AS_SUB_MAP_IN_TYPED_MAP_TABLE_DATA,
            EXPLICIT_RAW_MAP_MODEL_AS_SUB_MAP_IN_TYPED_MAP_ITEM_DATA,
            "map_id",
            "N",
        ...
        with patch(PATCH_METHOD, new=fake_db):
            ...
            self.assertEqual(sub_attr.map_field["floaty"], map_native.get("floaty"))
            self.assertEqual(
                actual.map_field["mapy"]["baz"], map_native.get("mapy").get("baz")
            )
    ...
    async def test_model_subclass_attributes_inherited_on_create(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assert_dict_lists_equal(
                actual["KeySchema"], DOG_TABLE_DATA["Table"]["KeySchema"]
            )
            self.assert_dict_lists_equal(
    ...
    async def test_model_version_attribute_save_with_initial_version_zero(self):
        ...
        with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
            ...
            params = {
                "Item": {
                    "name": {"S": "test_user_name"},
                    "email": {"S": "test_user@email.com"},
                    "version": {"N": "1"},
                },
                "ReturnConsumedCapacity": "TOTAL",
...
class ModelInitTestCase(IsolatedAsyncioTestCase):
    ...
    def test_subclassed_map_attribute_with_dict_init(self):
        attribute = {"make": "Volkswagen", "model": "Super Beetle"}
    ...
    def _get_bin_tree(self, multiplier=1):
        return {
            ...
            "left": {
                "value": 2 * multiplier,
                "left": {"value": 1 * multiplier},
                "right": {"value": 3 * multiplier},
            },
            "right": {
                "value": 7 * multiplier,
                "left": {"value": 6 * multiplier},
                "right": {"value": 8 * multiplier},
            },
        }
    ...
    def test_deserialized_with_ttl(self):
        m = TTLModel.from_raw_data(
            {"user_name": {"S": "mock"}, "my_ttl": {"N": "1546300800"}}
        )
...
@pytest.mark.parametrize("add_version_condition", [True, False])
@pytest.mark.asyncio
async def test_version_attribute_increments_on_update(
    add_version_condition: bool,
) -> None:
    item = VersionedModel("test_user_name", email="test_user@email.com")
    ...
    with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
        ...
        await item.update(
            actions=[VersionedModel.email.set("new@email.com")],
    ...
    with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
        req.return_value = {
            ATTRIBUTES: {
                "name": {"S": "test_user_name"},
                "email": {"S": "newer@email.com"},
                "version": {"N": "2"},
            }
        }

        await item.update(
            actions=[VersionedModel.email.set("newer@email.com")],
            add_version_condition=add_version_condition,
        )
        args = req.call_args[0][1]
        expected = {
...
async def test_delete(add_version_condition: bool) -> None:
    ...
    with patch(PATCH_METHOD, new_callable=AsyncMock) as req:
        req.return_value = None
        await item.delete(
            UserModel.user_id == "bar", add_version_condition=add_version_condition
        )
        expected = {
            "Key": {"user_id": {"S": "bar"}, "user_name": {"S": "foo"}},
            "ConditionExpression": "#0 = :0",
            "ExpressionAttributeNames": {"#0": "user_id"},
            "ExpressionAttributeValues": {":0": {"S": "bar"}},
            "ReturnConsumedCapacity": "TOTAL",
            "TableName": "UserModel",
        }
        args = req.call_args[0][1]
