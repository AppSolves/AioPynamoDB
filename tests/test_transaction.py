...
MOCK_TABLE_DESCRIPTOR = {
    "Table": {
        "TableName": "mock",
        "KeySchema": [
            {"AttributeName": "mock_hash", "KeyType": "HASH"},
            {"AttributeName": "mock_range", "KeyType": "RANGE"},
        ],
        "AttributeDefinitions": [
            {"AttributeName": "mock_hash", "AttributeType": "N"},
            {"AttributeName": "mock_range", "AttributeType": "N"},
        ],
    }
}
...
class TestTransactGet:
    ...
    async def test_commit(self, mocker):
        ...
        mock_connection_transact_get = mocker.patch.object(
        ...
        mock_connection_transact_get.assert_called_once_with(
            get_items=[
                {
                    "Key": {"mock_hash": {"N": "1"}, "mock_range": {"N": "2"}},
                    "TableName": "mock",
                }
            ],
...
class TestTransactWrite:
    ...
    async def test_commit(self, mocker):
        ...
        async with TransactWrite(connection=connection) as t:
            t.condition_check(
                MockModel, 1, 3, condition=(MockModel.mock_hash.does_not_exist())
            )
