...
class TestDefault:
    def test_default(self):
        ...
        with pytest.raises(
            ValueError, match="'default' must be immutable (.*) or a callable"
        ):
            Attribute(default=[])

        with pytest.raises(
            ValueError, match="'default_for_new' must be immutable (.*) or a callable"
        ):
        ...
        with pytest.raises(
            ValueError,
            match="An attribute cannot have both default and default_for_new parameters",
        ):
...
class TestUTCDateTimeAttribute:
    ...
    def test_utc_date_time_deserialize_pre_1000_not_padded(self):
        assert self.attr.deserialize("1-01-06T08:21:30.002000+0000") == self.dt.replace(
            year=1
        )
    ...
    def test_utc_date_time_invalid(self, invalid_string):
        with pytest.raises(
            ValueError, match=r"does not match format '%Y-%m-%dT%H:%M:%S.%f\+0000'"
        ):
...
class TestBinaryAttribute:
    ...
    @pytest.mark.parametrize(
        ["legacy_encoding", "serialized"],
        [
            pytest.param(
                False, b"foo", id="encoded from API, decoded in _covert_binary"
            ),
            pytest.param(
                True,
                b"Zm9v",
                id="double-encoded from API, once decoded in _convert_binary",
            ),
        ],
    )
...
class TestBinarySetAttribute:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                False,
                {b"foo", b"bar"},
                id="encoded from API, decoded in _covert_binary",
            ),
            pytest.param(
                True,
                [b"Zm9v", b"YmFy"],
    ...
    def test_binary_set_attribute(
        self, legacy_encoding: bool, expected: Set[bytes]
    ) -> None:
    ...
    def test_binary_set_serialize(
        self, legacy_encoding: bool, expected: List[bytes]
    ) -> None:
        """
    ...
    def test_binary_set_deserialize(
        self, legacy_encoding: bool, serialized: List[bytes]
    ) -> None:
        """
...
class TestNumberAttribute:
    ...
    def test_number_serialize(self):
        ...
        assert (
            attr.serialize(12345678909876543211234234324234)
    ...
    def test_number_deserialize(self):
        ...
        assert (
            attr.deserialize("12345678909876543211234234324234")
            == 12345678909876543211234234324234
        )
...
class TestTTLAttribute:
    ...
    def test_default_and_default_for_new(self):
        with pytest.raises(
            ...
            TTLAttribute(
                default=timedelta(seconds=1), default_for_new=timedelta(seconds=2)
            )
    ...
    def test_ttl_attribute_wrong_type(self):
        with pytest.raises(
            ValueError, match="TTLAttribute value must be a timedelta or datetime"
        ):
            model = AttributeTestModel()
    ...
    def test_serialize_deserialize(self, mock_time):
        ...
        assert s == "1559692860"
        assert TTLAttribute().deserialize(s) == datetime(
            2019, 6, 5, 0, 1, 0, tzinfo=timezone.utc
        )
...
class TestMapAttribute:
    ...
    def test_serialize_invalid_set(self):
        ...
        with pytest.raises(
            ValueError,
            match="Cannot serialize set consisting of types: <class 'int'>, <class 'str'>",
        ):
            attr.serialize({"test": {42, "foo"}})
        with pytest.raises(
            ValueError,
            match="Cannot serialize set consisting of types: "
    ...
    def test_null_attribute_subclassed_map(self):
        null_attribute = {"map_field": {}, "string_set_field": None}
        attr = DefaultsMap()
        serialized = attr.serialize(null_attribute)
    ...
    def test_map_of_map(self):
        attribute = {"name": "Justin", "metrics": {"age": 31, "height": 187.96}}
        attr = MapAttribute()
    ...
    def test_map_overridden_attrs_accessors(self):
        attr = CustomAttrMap(
            **{"overridden_number_attr": 10, "overridden_unicode_attr": "Hello"}
        )
    ...
    def test_map_overridden_attrs_serialize(self):
        attribute = {"overridden_number_attr": 10, "overridden_unicode_attr": "Hello"}
    ...
    def test_additional_attrs_deserialize(self):
        raw_data = {
            "number_attr": {"N": "10"},
            "unicode_attr": {"S": "Hello"},
            "undeclared_attr": {"S": "Goodbye"},
        }
        expected = {"overridden_number_attr": 10, "overridden_unicode_attr": "Hello"}
        assert CustomAttrMap().deserialize(raw_data).attribute_values == expected

    def test_defaults(self):
        item = DefaultsMap()
        assert item.validate()
        assert DefaultsMap().serialize(item) == {"map_field": {"M": {}}}
    ...
    def test_raw_map_from_dict(self):
        item = AttributeTestModel(
            raw_map_attr={"foo": "bar", "num": 3, "nested": {"nestedfoo": "nestedbar"}}
        )
    ...
    def test_raw_map_json_serialize(self):
        ...
        serialized_attr_from_raw = json.dumps(
            AttributeTestModel(raw_map_attr=raw).raw_map_attr.as_dict(), sort_keys=True
        )
    ...
    def test_typed_and_raw_map_json_serialize(self):
        ...
        item = SomeModel(typed_map=TypedMap(map_attr={"foo": "bar"}))

        assert json.dumps({"map_attr": {"foo": "bar"}}) == json.dumps(
            item.typed_map.as_dict()
        )
    ...
    def test_serialize_datetime(self):
        ...
        expected_serialized_value = {
            "date_attr": {"S": "2017-01-01T00:00:00.000000+0000"}
        }
    ...
    def test_complex_map_accessors(self):
        ...
        t = ThingModel(
            nested=NestedThing(
                double_nested={"hello": "world"}, double_nested_renamed={"foo": "bar"}
            )
        )
        ...
        with pytest.raises(AttributeError):
            t.nested.double_nested.bad
        with pytest.raises(AttributeError):
            t.nested.bad
        with pytest.raises(AttributeError):
            t.nested.something_else
    ...
    def test_attribute_paths_wrapping(self):
        ...
        assert mid_map_a_map_attr.attr_path == [
            "dyn_out_map",
            "mid_map_a",
            "dyn_in_map_a",
            "dyn_map_attr",
        ...
        assert mid_map_b_map_attr.attr_path == [
            "dyn_out_map",
            "mid_map_b",
            "dyn_in_map_b",
            "dyn_map_attr",
...
class TestDynamicMapAttribute:
    ...
    def test_deserialize(self):
        ...
        assert test_model.test_map.created_at == datetime(
...
class TestListAttribute:
    ...
    def test_list_type_error(self):
        ...
        with pytest.raises(
            ValueError, match="List elements must be of type: UnicodeAttribute"
        ):
            string_list_attribute.serialize([MapAttribute(foo="bar")])

        with pytest.raises(
            TypeError, match=r"Cannot deserialize '\[0\]' attribute from type: S"
        ):
    ...
    def test_serialize_null(self):
        ...
        assert string_set_list_attribute.deserialize(serialized) == [
            {"foo"},
            None,
            None,
        ]
...
class TestMapAndListAttribute:
    ...
    def test_map_of_list_of_map(self):
        family_attributes = {
            "phillips": [
                {"name": "Justin", "age": 31, "height": 187.96},
                {"name": "Galen", "age": 29, "height": 193.04, "male": True},
                {
                    "name": "Colin",
                    "age": 32,
                    "height": 203.2,
            ...
            "workman": [
                {"name": "Mandy", "age": 29, "height": 157.48, "female": True},
                {"name": "Rodney", "age": 31, "height": 175.26, "hasChild": False},
            ],
        }
...
class TestNormalDict:
    ...
    def test_to_simple_dict__not_force(self):
        ...
        with pytest.raises(ValueError):
            DictTestModel(
                raw_map_attr={"string": "bar", "binary": b"baz"}
            ).to_simple_dict(force=False)

        with pytest.raises(ValueError):
            DictTestModel(
                DictTestMapAttribute(string="bar", binary=b"foo")
            ).to_simple_dict(force=False)
...
class TestDynamoDBDict:
    def test_to_dynamodb_dict(self):
        ...
        assert actual == {
            "binary_attr": {"B": "Zm9v"},
            "binary_set_attr": {"BS": ["Zm9v", "YmFy"]},
            "bool_attr": {"BOOL": True},
            "datetime_attr": {"S": "2022-12-31T23:59:59.000000+0000"},
            "json_attr": {"S": '{"foo": "bar"}'},
            "list_attr": {
                "L": [
                    {"M": {"binary": {"B": "Zm9v"}, "string": {"S": "bar"}}},
                ],
            },
            "map_attr": {"M": {"binary": {"B": "Zm9v"}, "string": {"S": "bar"}}},
            "null_attr": {"NULL": True},
            "number_attr": {"N": "1"},
            "number_set_attr": {"NS": ["1", "2", "3"]},
            "raw_map_attr": {"M": {"binary": {"B": "Zm9v"}, "string": {"S": "bar"}}},
            "ttl_attr": {"N": "1672531199"},
            "unicode_attr": {"S": "foo"},
            "unicode_set_attr": {"SS": ["foo", "bar"]},
        }
